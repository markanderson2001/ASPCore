    ...
    public class WorldContextSeedData
    {
        ...
        public async Task EnsureSeedData()
        {
                ...
                if (!_context.Trips.Any())  //any boolean if any objects in it, could also put a lambda here. Add the ! so if there arent any trips then;
            {
                ...
                _context.Trips.Add(usTrip); //call to add data

                _context.Stops.AddRange(usTrip.Stops); //add to stop list)
            ...
            new Stop() { Order = 55, Latitude =  33.748995, Longitude =  -84.387982, Name = "Atlanta, Georgia", Arrival = DateTime.Parse("Jun 17, 2015") },
                ...
                _context.Trips.Add(worldTrip); //call to add data

                _context.Stops.AddRange(worldTrip.Stops); //add to stop list)
